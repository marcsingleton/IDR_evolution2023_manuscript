...
\textbf{(A)} Number of disorder and order regions with zero and non-zero overlap with any Pfam domain. The proportion of regions with no overlap is significantly greater for the disorder regions ($p < 1 \times 10^{-10}$, chi-squared test). \textbf{(B)} Histogram of the disorder and order regions' overlaps with any Pfam domain. Regions with zero overlap are excluded to more clearly show the distribution.}
\label{sfig:pfam}
\end{figure}
